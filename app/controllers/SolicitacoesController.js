...
const createSolicitacoes = async (req, res) => {
  try {
    ...
    if (!ciencia_substituto || ciencia_substituto == "") {
      res.json({
        success: false,
        message: "Complete the ciencia_substituto field",
      });
    }
    if (!data_devolucao || data_devolucao == "") {
      res.json({
        success: false,
        message: "Complete the data_devolucao field",
      });
    }
    if (!hora_devolucao || hora_devolucao == "") {
      res.json({
        success: false,
        message: "Complete the hora_devolucao field",
      });
    }
    if (!autorizacao_chefia || autorizacao_chefia == "") {
      res.json({
        success: false,
        message: "Complete the autorizacao_chefia field",
      });
    }
    if (!ciencia_chefia || ciencia_chefia == "") {
      res.json({
        success: false,
        message: "Complete the ciencia_chefia field",
      });
    }
    if (!ciencia_coordenador || ciencia_coordenador == "") {
      res.json({
        success: false,
        message: "Complete the ciencia_coordenador field",
      });
    }
    if (!docente_solicitante_id || docente_solicitante_id == "") {
      res.json({
        success: false,
        message: "Complete the docente_solicitante_id field",
      });
    }
    if (!docente_substituto_id || docente_substituto_id == "") {
      res.json({
        success: false,
        message: "Complete the docente_substituto_id field",
      });
    }
    ...
    if (!disciplina_substituta_id || disciplina_substituta_id == "") {
      res.json({
        success: false,
        message: "Complete the disciplina_substituta_id field",
      });
    }
    ...
    if (!coordenador_id || coordenador_id == "") {
      res.json({
        success: false,
        message: "Complete the coordenador_id field",
      });
    }
...
const alterSolicitacao = async (req, res) => {
  try {
    const {
      data,
      hora,
    ...
    const createdSolicitacao = await Solicitacao.update(
      {
        data,
        hora,
        justificativa,
    ...
    if (createdSolicitacao[0] === 1) {
      res.json({ success: true, message: "Solicitacao updated successfully" });
    } else {
      res.json({ success: false, message: "Solicitacao not found or not updated" });
    }
  } catch (error) {
    res.status(201).json({
      success: false,
